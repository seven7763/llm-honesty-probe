...
- **空回复说明（v0.2.1）**：推理型模型会用 completion token 先思考再作答。调用方给的 `--max-tokens` 太小时，旧版会把这种「预算饿死」误判为能力失败 —— 那是调用方参数问题，不是服务不诚实。新版会识别该形态并报告为"无法判定"并提示加大预算，小预算不再能伪造出 SUSPICIOUS 判定。
...
- **免费额度与网关分组会让探针"失明"**：用免费 key（或网关分组里没有该模型的可用渠道）跑长上下文、格式、拒绝行为三项，常拿回配额/限流/模型不可用类错误。这类信号一律记为"无法判定"并打上 `free-tier-limited` 标注——"探针测不了"不等于"端点有问题"。带此标注的 PASS 只说明：该额度给得出答案的部分表现一致；免费组的行为未必等于你付费的分组，请对准你实际购买的那组再跑一遍。
- **本工具曾公开地错过一次**：我们把 v0.2.0 对准自家网关的一个分组，得到了错误的 SUSPICIOUS——完整经过（含原始报告与修复）见[自测事故记录](docs/incident-2026-09-04-self-test.md)。读它能校准"红旗"和"PASS"各值多少。
